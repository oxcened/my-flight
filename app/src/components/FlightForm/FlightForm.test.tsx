...
describe(FlightForm.name, () => {
  it('Should render disabled submit btn when loading', () => {
    ...
    expect(screen.getByRole('button', { name: "Let's take off!" })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: "Let's take off!" })).toBeDisabled();
  });
  ...
  it('Should render enabled submit btn when not loading', () => {
    ...
    expect(screen.getByRole('button', { name: "Let's take off!" })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: "Let's take off!" })).toBeEnabled();
  });
  ...
  it('Should not trigger onSubmit when submit btn is clicked and form is invalid', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: "Let's take off!" }));
    expect(handleSubmit).not.toHaveBeenCalled();
  });

  it('Should select departure airport when clicked', async () => {
    render(<FlightForm />);
    await userEvent.selectOptions(screen.getByLabelText('Select Departure'), ['Malpensa Airport (MXP)']);
  ...
  it('Should not select arrival airport when clicked', async () => {
    render(<FlightForm />);
    await userEvent.selectOptions(screen.getByLabelText('Select Arrival'), ['Malpensa Airport (MXP)']);
  ...
  it('Should trigger onSubmit when submit btn is clicked and form is valid', async () => {
    ...
    await userEvent.selectOptions(screen.getByLabelText('Select Departure'), ['Malpensa Airport (MXP)']);
    await userEvent.selectOptions(screen.getByLabelText('Select Arrival'), ['Barcelona Airport (BCN)']);
